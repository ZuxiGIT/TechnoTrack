...
static const char* type2str(node_type_t type)
{
    switch(type)
    {
        case FUNCTION:
        ...
        case UNDEFINED:
            return "Undefined";
        default:
            return "Uknown type of node";
            /*
            pr_err(LOG_CONSOLE, "Undefined node\n");
            exit(EXIT_FAILURE);
            */
    }
}
...
static void* _create_node(node_type_t type, value_t value, Node* parent, 
                          Node* left, Node* right)
{
       ...
       type == FUNC_CALL)
    {
        ...
        res = (Func_node*)calloc(1, sizeof(Func_node));

        res->id_base.value = value;
        res->id_base.node_base.left = left;
        res->id_base.node_base.right = right;
        res->id_base.node_base.type = type;
            ...
            type == VARIABLE_DEF)
    {
        ...
        res = (Id_node*)calloc(1, sizeof(Id_node));

        res->node_base.left = left;
        res->node_base.right = right;
        res->node_base.type = type;

        res->value = value;

        return res;
    }
    else if(type == BLOCK_OF_STATEMENTS)
    {
        Blk_node* res = NULL;

        res = (Blk_node*)calloc(1, sizeof(Blk_node));
...
inline void* create_node(node_type_t type, value_t value)
{
...
static void printNode(const Node* node)
{
    printf("\n{\n\ttype: %d(%s)\n\taddress: %p\n\tright: %p\n\tleft: %p\n"
           "\tparent: %p\n",
           node->type, type2str(node->type), node, node->right, node->left, 
           node->parent);
...
Node* copy_node(const Node* node)
{
    ...
    Node* res = create_node(node->type, (value_t)((char*)NULL));

    if( (node->type == VARIABLE) || 
        (node->type == FUNCTION) ||
        (node->type == FUNC_CALL))
    {
        ((Id_node*)res)->value.text = strndup(((Id_node*)node)->value.text, 
                                              strlen(((Id_node*)node)->value.text));
        ((Id_node*)res)->alloc_name = true;
    }
    else
        memcpy(&(((Id_node*)res)->value.num), 
               &(((Id_node*)node)->value.num), sizeof(double));
...
void node_free(Node* node)
{
    ...
    if(node->type == FUNCTION)
    {
        id_table_free(((Func_node*)node)->id_table);
        free(((Func_node*)node)->id_table);
    }

    if(node->type == BLOCK_OF_STATEMENTS)
    {
        id_table_free(((Blk_node*)node)->id_table);
        free(((Blk_node*)node)->id_table);
    }
...
/* let it be deprecated for a while, i will correct it later maybe :)
 * i think tex dump is useless for our purpose, but who knows, who knows ...
 *
...
static int _save_node(Node* node, char* dump_buff, int buff_pos, 
                      int shift, int side)
{
    if(node == NULL)
    {
    ...
    buff_pos += sprintf(curr_pos, "type: %d (%s)\n", node->type,
                                  type2str(node->type));
    ...
    if(node->type == CONSTANT) 
        ...
        buff_pos += sprintf(curr_pos, "value: #\n");
    else if(node->type == BLOCK_OF_STATEMENTS)
        buff_pos += sprintf(curr_pos, "value: @\n");
    else if(node->type == SEMICOLON)
        buff_pos += sprintf(curr_pos, "value: ;\n");
    else if(node->type == CONDITION)
        buff_pos += sprintf(curr_pos, "value: \"if\"\n"); 
    else if(node->type == FUNC_CALL)
        buff_pos += sprintf(curr_pos, "value: \"%s\"\n",
                                      ((Func_node*)node)->id_base.value.text);
    else
    {
        pr_err(LOG_CONSOLE, "Bad node type to save\n");
        return -1;
    }
...
static Node* _parse_node_from_save(Tree* tree, char** text)
{
    ...
    if(*txt == '"')
    {
        ...
        {
            //printf("%s\n", txt); 
            pr_err(LOG_CONSOLE, "Bad node type [%d (%s)]\n",
                                node->type, type2str(node->type));
            free(node);
            return NULL;
        }
    ...
    else if(isdigit(*txt))
    {
        if(node->type == CONSTANT)
        {
            sscanf(txt, "%lf", &(((Id_node*)node)->value.num));
        ...
        {
            pr_err(LOG_CONSOLE, "Bad node format: bad value"
                                " for CONSTANT node\n");
            free(node);
            return NULL;
        }
...
Tree* load_tree(const char* input)
{
    ...
    if(sz < 0)
    {
        ...
        pr_err(LOG_CONSOLE, "Bad input filepath [filepath: \"%s\"]"
                            " for loading\n",
                            input);
        freopen(NULL, "w", stdout);
        return NULL;
    ...
    if(sz == 0)
    {
        pr_err(LOG_CONSOLE, "Bad input file [file: %s] [size = 0]"
                            " for loading\n",
                            input);
        return NULL;
    }
